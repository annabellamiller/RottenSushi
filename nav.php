<div class='navbar'>
  <div class="navbar-right">
    <a href='/rottensushi/login/logout.php'>Logout</a>
    <a href='/rottensushi/crud-Purchase/view-purchase.php'>My Movies</a>
    <a href='/rottensushi/cart/cart.php'>Cart</a>
    <a href='/rottensushi/crud-Actor/add-actor.php'>Add Actor</a>
    <a href='/rottensushi/crud-Actor/view-actor.php'>View Actors</a>
    <a href='/rottensushi/crud-Movie/add-movie.php'>Add Movie</a>
    <a href='/rottensushi/crud-Review/add-review.php'>Add Review</a>
  </div>
  <div class="navbar-left">
    <a href='/rottensushi/home.php'>Rotten Sushi</a>
    <a href='/rottensushi/crud-User/view-user.php'>
      <div class="logo-image">
        <img src='\RottenSushi\Images\logo.jpg' alt='Rotten Sushi Logo' style="display: flex; align-items: center; width: 30px; height: auto; margin-right: 10px; padding:0px;">
      </div>
    </a>
  </div>
</div>
